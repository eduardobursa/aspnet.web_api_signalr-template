...
namespace Template.WebApi
{
	...
	public partial class Startup
	{
		...
		public static void ConfigureSwagger(HttpConfiguration config)
		{
			...
			config
				...
				.EnableSwaggerUi(c =>
				{
					...
					c.CustomAsset("index", thisAssembly, "Template.WebApi.Content.html.swagger.html");
		...
		{
			return BuildSwaggerDocumentationFile();
		}

		/// <summary>
		/// 
		/// </summary>
		private static string BuildSwaggerDocumentationFile()
		{
			XElement xml = null;
			XElement dependentXml = null;
			string docsDirectory = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "bin");

			foreach (string documentationFilePath in Directory.GetFiles(docsDirectory, "Template.*.xml"))
			{
				if (xml == null)
				{
					xml = XElement.Load(documentationFilePath);
				}
				else
				{
					dependentXml = XElement.Load(documentationFilePath);
			...
			string swaggerFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "App_Data", "SwaggerDocumentation.xml");
